...
```dax
Чистая выручка = SUMX(Sales,Sales[SalesAmount]-Sales[ReturnAmount]-Sales[DiscountAmount])
Количество заказов = COUNTROWS(Sales)
Валовая прибыль = [Выручка] - SUMX(Sales, Sales[TotalCost]-Sales[ReturnQuantity]*Sales[UnitCost])
Сумма возвратов = SUM(Sales[ReturnAmount])
Валовая маржа = DIVIDE([Валовая прибыль],[Выручка])
```

## Ответы на вопросы и выводы на основе дашборда

- Основной канал продаж — офлайн-магазины (58% прибыли), онлайн даёт только 20%, каталоги с магазинами реселлерами приносят сопоставимую с онлайн магазинами 22% дохода
- Категории товаров "компьютеры" и "камеры" - самые прибыльные категории, на них приходится основная доля прибыли, а именно 69%
- Почти у всех брендов - особенно у крупных, которые приносят большую часть дохода - отношения суммы возвратов к части прибыли которые они приносят, составляет в основном не около 2%, что считается допустимым значением 
